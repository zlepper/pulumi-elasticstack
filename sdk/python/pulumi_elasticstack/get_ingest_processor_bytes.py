...
__all__ = [
    'GetIngestProcessorBytesResult',
    'AwaitableGetIngestProcessorBytesResult',
    'get_ingest_processor_bytes',
    'get_ingest_processor_bytes_output',
]

@pulumi.output_type
class GetIngestProcessorBytesResult:
    """
...
class AwaitableGetIngestProcessorBytesResult(GetIngestProcessorBytesResult):
    # pylint: disable=using-constant-test
    def __await__(self):
        if False:
            yield self
        return GetIngestProcessorBytesResult(
...
def get_ingest_processor_bytes(description: Optional[str] = None,
                               ...
                               opts: Optional[pulumi.InvokeOptions] = None) -> AwaitableGetIngestProcessorBytesResult:
    """
    ...
    __ret__ = pulumi.runtime.invoke('elasticstack:index/getIngestProcessorBytes:getIngestProcessorBytes', __args__, opts=opts, typ=GetIngestProcessorBytesResult).value

    return AwaitableGetIngestProcessorBytesResult(
...
@_utilities.lift_output_func(get_ingest_processor_bytes)
def get_ingest_processor_bytes_output(description: Optional[pulumi.Input[Optional[str]]] = None,
                                      ...
                                      opts: Optional[pulumi.InvokeOptions] = None) -> pulumi.Output[GetIngestProcessorBytesResult]:
    """
    Helper data source to which can be used to create a processor to convert a human readable byte value (e.g. 1kb) to its value in bytes (e.g. 1024). If the field is an array of strings, all members of the array will be converted.
